...
test("should let user request new access token", async() => {
    const loginResponse = await agent
        .post('/users/login')
        .send({
            username: dbFixture.secondUser.username,
    ...
    const userAccessToken = loginResponse.body.refreshToken
    const token = `Bearer ${userAccessToken}`
    ...
    expect(requestResponse.body.accessToken).not.toBeNull()
